...
class BaseController extends Controller
{
    ...
    {
        if(in_array($this->id.'/'.$this->action->id,\Yii::$app->params['noAuth'])){
        ...
        if(!User::find()->where(['rdSession'=>User::$_RD_SESSION])->exists()){
            BaseModule::error(-2);
        }
        User::$id = User::find()->select(['id'])->where(['rdSession'=>User::$_RD_SESSION])
            ->scalar();
